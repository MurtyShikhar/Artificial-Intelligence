...
float probability_data_point(Trainer& t, Observation& obs){
	assert (is_complete(obs));
	float ans = 0.0;
	for (int i = 0; i < obs.size(); i++){
		#define node t.network.Pres_Graph[i];
		int _case = t.which_case(obs, node);
		ans += log10(node.CPT[_case]);
		#undef node
	}
...
float probability_data_point_incomplete(Trainer& t, Observation& obs, int incomplete_node_index){
	assert (!is_complete(obs))
	float ans = 0.0;
	...
	for (int i = 0; i < node.nvalues; i++){
		...
		ans += probability_data_point(t, aux_obs);
	}
...
float log_likelihood(Trainer& t, vector<pair<int, int> > & incomplete_data_points){
	#define DATA t.data
	float val = 0.0;
	for (int i = 0; i < DATA.size(); i++){
		if (!is_complete(DATA[i]))
		{
			...
			val += probability_data_point_incomplete(t,DATA[i], incomplete_node_index);
		}

		else{
			val += probability_data_point(t, DATA[i]);
		}
	}
